...
class Inventaris_PC_Laboratorium_D extends Model
{
    protected $guarded = ['id'];

    protected $table='inventaris__p_c__laboratorium__d_s';

    protected static function boot()
    {
        parent::boot();

        static::creating(function ($invLABD) { //PARAMETER BEBAS
            $lastId = self::max('id') + 1; // Ambil ID terakhir & tambahkan 1
            $kodeUnik = str_pad($lastId, 2, '0', STR_PAD_LEFT); // Format 01, 02, dst.
            $invLABD->no_inventaris = 'UDN/LABKOM/INV/D2D/PC' . $kodeUnik;
        });
    }
