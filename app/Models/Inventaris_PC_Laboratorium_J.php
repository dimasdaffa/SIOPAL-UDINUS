...
class Inventaris_PC_Laboratorium_J extends Model
{
    protected $guarded = ['id'];

    protected $table='inventaris__p_c__laboratorium__j_s';

    protected static function boot()
    {
        parent::boot();

        static::creating(function ($invLABJ) { //PARAMETER BEBAS
            $lastId = self::max('id') + 1; // Ambil ID terakhir & tambahkan 1
            $kodeUnik = str_pad($lastId, 2, '0', STR_PAD_LEFT); // Format 01, 02, dst.
            $invLABJ->no_inventaris = 'UDN/LABKOM/INV/D2J/PC' . $kodeUnik;
        });
    }
